(function (exports, d3, aq, vega) {
  ...
  async function playSystemSpeech(sound, config) {
    return new Promise((resolve, reject) => {
      ...
      if (sound?.speechRate !== undefined) utterance.rate = sound?.speechRate;
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let sound of q) {
        ...
        inst.onended = async () => {
          ...
          if (sound?.timbre === SpeechTone) {
            await playSingleSpeechTone(sound, config);
          }
          else await playSingleTone(ctx, sound, config, instSamples, synthDefs, waveDefs, filters);
          if (sound.isLast) {
            if (config?.isRecorded) await playPause(300);
  ...
  async function playSingleSpeechTone(sound, config) {
  // clear previous state
  if (config?.subpart && ErieGlobalState === Stopped$1) return;
  if (!config?.subpart) ErieGlobalState = undefined;
  ...
  return new Promise((resolve, reject) => {
    if (!ErieGlobalSynth) ErieGlobalSynth = window.speechSynthesis;
    console.log("TEST")
    console.log(sound.others.SpeechToneText)
    ...
    if (sound?.others?.SpeechToneVoice !== undefined){
      ...
      let voiceIndex = Math.round(sound.others.SpeechToneVoice);
  ...
  const Types = [TextType, ToneType, SpeechToneType, ToneSeries, ToneOverlaySeries, Pause, ToneSpeechSeries, LegendType];
  ...
  class AudioGraphQueue {
    ...
    async playLine(item) {
      ...
      else if (item?.others?.timbre === SpeechToneType) {
        await playSingleSpeechTone(item, config);
      }
  ...
  class SequenceStream {
    ...
    async prerender() {
      ...
      if (!this.config.skipStartSpeech) {
        this.queue.add(TextType, { speech: ``, speechRate: this.config?.speechRate }, this.config);
      }
